...
public class Trans2PinYin {
    ...
    /** 
     * 转换为拼音（小写）,首位非字母 增加前缀 控制排序
     * @param str 
     * @param prefixIfNoLetter 转拼音后首位非字母 前缀
     * @return 
     */  
    public static String toPinYin(String str, String prefixIfNoLetter) {  
        String string = Trans2PinYin.getInstance().convertAll(str);
        ...
        if (StringUtils.isNotBlank(prefixIfNoLetter) && !StringUtils.isAlpha(string.substring(0, 1))) {
			return StringUtils.join(prefixIfNoLetter,string);
		}
    ...
    public static void main(String[] args) {
		String ss = "sAJKHKHdfujlskjdlfkjsdkSD卡解放路口……%&";
		
		System.out.println(Trans2PinYin.toPinYin(ss, "{"));
    ...
    public static String toPinYin2(String str) {
    	...
    	for (int i = 0; i < len; i++) {
    		...
    		ret += (Trans2PinYin.getInstance().convert(tmp) + "&");    		
    	}
    	return ret;
    }
}
